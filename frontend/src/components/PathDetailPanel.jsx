...
const PathDetailPanel = ({ pathId, onCreateRun, onNavigateToAspect, onDeleted, onUpdated }) => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setLoading(true);
      try {
        const [pathData, runData] = await Promise.all([getPath(pathId), getRunsByPath(pathId)]);
        setDetail(pathData);
        setRuns(runData);
      } catch {
        setDetail(null);
  ...
  const cancelEditPath = () => { setEditingPath(false); setPathForm(null); setPathSaveError(null); };
  ...
  const handleSavePath = async () => {
    ...
    try {
      const payload = {
        ...
        data_source_description: pathForm.data_source_description.trim() || null,
  ...
  const handleRemoveAspect = async (aspectId) => {
    ...
    try {
      ...
      setDetail((prev) => ({ ...prev, aspects: prev.aspects.filter((a) => a.id !== aspectId) }));
    } catch (err) {
  ...
  const handleAddAspect = async () => {
    ...
    setAddAspectLoading(true);
    try {
      const updated = await addAspectToPath(pathId, { aspect_id: selectedAspectToAdd }, token);
      setDetail(updated);
      setAddingAspect(false);
  ...
  return (
    <>
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "0.75rem 0 0.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.4rem" }}>
                <button type="button" onClick={cancelEditPath} style={secondaryBtn}>Cancel</button>
                <button type="button" onClick={handleSavePath}
                  disabled={savingPath || !pathForm.data_source_label.trim()}
                  style={{ ...primaryBtn, opacity: savingPath || !pathForm.data_source_label.trim() ? 0.5 : 1 }}>
                  {savingPath ? "Saving…" : "Save"}
                </button>
              </div>
            </div>
            <label style={labelStyle}>Data source label *</label>
            <input value={pathForm.data_source_label}
              onChange={(e) => setPathForm((p) => ({ ...p, data_source_label: e.target.value }))}
              style={{ ...inputStyle, marginBottom: "0.75rem" }} autoFocus />
            <label style={labelStyle}>Task description</label>
            <textarea value={pathForm.task_description}
              onChange={(e) => setPathForm((p) => ({ ...p, task_description: e.target.value }))}
              rows={3} style={{ ...inputStyle, resize: "vertical", marginBottom: "0.75rem" }} />
            <label style={labelStyle}>Data source description</label>
            <textarea value={pathForm.data_source_description}
              onChange={(e) => setPathForm((p) => ({ ...p, data_source_description: e.target.value }))}
              rows={3} style={{ ...inputStyle, resize: "vertical", marginBottom: "0.75rem" }} />
            {pathSaveError && <p style={{ color: "#e07070", fontSize: "0.8rem" }}>{pathSaveError}</p>}
          </>
        ) : (
      ...
      {deleteError && <p style={{ color: "#e07070", fontSize: "0.8rem", marginBottom: "1rem" }}>{deleteError}</p>}
      ...
      {/* Recommended aspects */}
      <p className="is-size-7 is-uppercase has-text-grey"
        style={{ letterSpacing: "0.1em", marginBottom: "0.75rem" }}>
        Recommended Aspects
      </p>

      {removeAspectError && (
        <p style={{ color: "#e07070", fontSize: "0.8rem", marginBottom: "0.75rem" }}>{removeAspectError}</p>
      )}
      ...
      {detail.aspects.length === 0 ? (
        ...
        <div className="columns is-multiline" style={{ marginBottom: "1rem" }}>
            ...
            <div key={aspect.id} className="column is-6">
              {editingAspectId === aspect.id ? (
                <PathAspectEditForm
                  ...
                  onSave={(updatedDetail) => { setDetail(updatedDetail); setEditingAspectId(null); }}
                ...
                <>
                  ...
                  >
                    ...
                    {aspect.definition && <p className="is-size-7">{aspect.definition}</p>}
                  </PathAspectCard>
                  {isAdmin && (
                    <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.4rem" }}>
                      <button type="button" onClick={() => setEditingAspectId(aspect.id)}
                        style={secondaryBtn}>
                        Edit
                      </button>
                      <button type="button"
                        ...
                        title={!canRemoveAspect(aspect) ? "Metrics used in a run — cannot remove" : "Remove aspect"}
                        style={{ ...secondaryBtn, opacity: !canRemoveAspect(aspect) ? 0.4 : 1, cursor: !canRemoveAspect(aspect) ? "not-allowed" : "pointer" }}>
                        Remove
                      </button>
                    </div>
                  )}
        ...
        <div style={{ marginBottom: "2rem" }}>
          {addingAspect ? (
            <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
              <select value={selectedAspectToAdd}
                onChange={(e) => setSelectedAspectToAdd(e.target.value)}
                style={{ ...inputStyle, flex: 1 }}>
                <option value="" disabled>Select an aspect to add…</option>
              ...
              <button type="button" onClick={handleAddAspect}
                ...
                style={{ ...primaryBtn, opacity: !selectedAspectToAdd || addAspectLoading ? 0.5 : 1 }}>
                {addAspectLoading ? "Adding…" : "Add"}
              </button>
              <button type="button" onClick={() => setAddingAspect(false)} style={secondaryBtn}>
                Cancel
              </button>
            </div>
          ) : (
            <button type="button" onClick={startAddingAspect} disabled={addAspectLoading}
              style={{ background: "rgba(255,196,81,0.1)", border: "1px dashed rgba(255,196,81,0.4)", color: "#ffc451", borderRadius: "4px", padding: "0.4rem 1rem", fontSize: "0.78rem", cursor: "pointer" }}>
      ...
      <p className="is-size-7 has-text-grey" style={{ marginBottom: "1rem" }}>
        ...
        <button type="button" className="button is-ghost is-small"
          ...
          {runs.map((run) => (
            <div key={run.id} className="column is-6">
              <RunCard
                ...
                onNavigate={() => navigate(`/use-cases/${detail.use_case_id}/${run.path_id}/${run.id}`)}
              />
            </div>
          ))}
        </div>
